...
log_interval = 10
l2_decay = 5e-4
root_path = "dataset/office31_raw_image/Original_images/"
source1_name = "amazon"
source2_name = "dslr"
target_name = "webcam"
...
def train(model):
    ...
    for i in range(1, iteration + 1):
        ...
        cls_loss, mmd_loss, ldr1, ldr2, ldr3, _ = model(source_data, target_data, source_label)
        gamma = 2 / (1 + math.exp(-10 * (i) / (iteration) )) - 1
        loss = cls_loss + gamma * mmd_loss + ldr1 + ldr2 + ldr3
        loss.backward()
        optimizer.step()

        if i % log_interval == 0:
            print('Train source1 iter: {} [({:.0f}%)]\tLoss: {:.6f}\tsoft_Loss: {:.6f}\tmmd_Loss: {:.6f}\tSparse_Loss1: {:.6f}\tSparse_Loss2: {:.6f}\tSparse_Loss3: {:.6f}\t'.format(
                i, 100. * i / iteration, loss.item(), cls_loss.item(), mmd_loss.item(), ldr1.item(), ldr2.item(), ldr3.item()))
